...
export default function BottomModuleBar({ onConfigureRoute }: BottomModuleBarProps) {
  ...
  const handleAddNewRouter = () => {
    ...
    addRouterPanel({
      ...
      targetOutputGroupId: outputGroups[0]?.id || null,
      active: true,
      visible: true,
      focused: true
    });
    window.dispatchEvent(
      new CustomEvent('simpleworship:notify', { 
        detail: `Opened new Router Panel ${nextIndex}` 
      })
    );
  };
          ...
          {routerPanels.map((router, index) => {
            ...
            return (
              <button
                ...
                title={`Bring Router Panel ${index + 1} to front`}
              >
                ...
                <span className={`truncate max-w-[140px] uppercase tracking-wide ${
                  ...
                  R-{index + 1}: {targetGroup?.name || 'No Target'}
                </span>
