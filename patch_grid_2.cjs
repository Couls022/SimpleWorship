const fs = require('fs');
let code = fs.readFileSync('src/components/LivePanel.tsx', 'utf8');

const target1 = `              ) : (
                slides.map((slide, idx) => {
                  const isSelected = activeControlState?.activeSlideIndex === idx;`;

const replace1 = `              ) : (
                <div 
                  className="grid gap-1.5 p-1.5"
                  style={{ gridTemplateColumns: viewMode === 'summary' ? '1fr' : 'repeat(auto-fill, minmax(240px, 1fr))' }}
                >
                {slides.map((slide, idx) => {
                  const isSelected = activeControlState?.activeSlideIndex === idx;`;

if(code.includes(target1)) { code = code.replace(target1, replace1); console.log('Replaced target1'); }

const target2 = `                ) : (
                  slides.map((slide, idx) => {
                    const isSelected = activeControlState?.activeSlideIndex === idx;`;

const replace2 = `                ) : (
                  <div 
                    className="grid gap-1.5 p-1.5"
                    style={{ gridTemplateColumns: viewMode === 'summary' ? '1fr' : 'repeat(auto-fill, minmax(240px, 1fr))' }}
                  >
                  {slides.map((slide, idx) => {
                    const isSelected = activeControlState?.activeSlideIndex === idx;`;

if(code.includes(target2)) { code = code.replace(target2, replace2); console.log('Replaced target2'); }


const target2b = `                      return (
                        <div
                          key={slide.id || idx}
                          onClick={() => handleSelectSlide(idx)}
                          className={\`flex cursor-pointer transition-all \${
                            viewMode === 'summary' 
                              ? 'py-0.5 border-b border-[#22242c]' 
                              : 'mb-1 border rounded-md overflow-hidden shadow-xs mx-1 first:mt-1'
                          }`;
const replace2b = `                      return (
                        <div
                          key={slide.id || idx}
                          onClick={() => handleSelectSlide(idx)}
                          className={\`flex cursor-pointer transition-all \${
                            viewMode === 'summary' 
                              ? 'py-0.5 border-b border-[#22242c]' 
                              : 'border rounded-md overflow-hidden shadow-xs'
                          }`;
if(code.includes(target2b)) { code = code.replace(target2b, replace2b); console.log('Replaced target2b'); }


const target1c = `                          </div>
                        )}
                      </div>
                    </div>
                  );
                })
              )}
            </div>
          </div>
        </div>
      </div>
    );
  }`;
const replace1c = `                          </div>
                        )}
                      </div>
                    </div>
                  );
                })}
                </div>
              )}
            </div>
          </div>
        </div>
      </div>
    );
  }`;

if(code.includes(target1c)) { code = code.replace(target1c, replace1c); console.log('Replaced target1c'); }

fs.writeFileSync('src/components/LivePanel.tsx', code);
console.log('DONE!');
